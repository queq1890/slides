...
| ボリューム | 25 枚前後（実績はセクション扉込みで 30 枚。2026-08-20 のフィードバックで既存ライブラリ・デモ③・Java 由来の話を削除し、タイムライン再配置と Safari スライドを追加して同数） |
...
- サポート状況セクションは**フル版**（2026-08-20 再構成）: マトリクス → Baseline の定義 → 9 年タイムライン → なぜ①仕様が巨大 → なぜ② Stage 3 でも仕様変更が続いた（Stage 2.7 新設の話込み）→ Safari はなぜ最後になったのか → いつ使えるか → polyfill 2 枚。タイムラインを「なぜ」より先に置くのは、経過年数を先に見せないと「なぜ時間がかかったのか」が唐突になるため（2026-08-20 フィードバック）
- **V8 の Rust 実装（temporal_rs）スライドと web.dev 引用は 2026-08-20 のフィードバックで削除**
- npm パッケージの**独立セクションは廃止**（2026-08-20）: polyfill 2 枚をセクション 03 の末尾に統合し、実装デモがセクション 04 に。既存ライブラリのスタンス表も削除済み
- デモは 2 本（タイムゾーン変換・型選択）。デモ③（ネイティブ実演）と「学ぶなら今がちょうどいい」系の締め文言は削除
- 用語: 「出荷（済み）」は **ship / shipped**、「夏時間」は**サマータイム**、「提案」は **Proposal** と表記（2026-08-20 フィードバック）
...
### Safari の経緯（2026-08-20 の裏取りで判明）

発表者の当初認識「Safari は一度実装を ship したが仕様変更が多く安定版にできていない」は、**「ship」が安定版出荷の意味なら誤り**（安定版には一度も入っていない）。正確な経緯は次のとおりで、スライド「Safari はなぜ最後になったのか」に反映済み:

- 2021-03（Stage 3 直後）から JSC にフラグ付き（`--useTemporal`）の部分実装が存在（WebKit bug 223166、起票は Apple の Yusuke Suzuki 氏）
- 完成しないまま停滞。2024-06 の仕様縮小で、初期実装が作り込んでいた `Temporal.Calendar` / `Temporal.TimeZone` が削除対象になり手戻り
- 2025 年に Igalia（Tim Chevalier 氏）が実装を再開、約 40 PR で完成へ（https://blogs.igalia.com/compilers/2026/02/02/implementing-the-temporal-proposal-in-javascriptcore/）
- 2026-07-29 の STP 249 で初めてデフォルト有効化。安定版 26.0〜26.6・Safari 27 beta には未搭載（2026-08-20 確認）
- STP 249 の実装は JSC 自前の C++（temporal_rs はコメントで参照設計として言及されるのみ）
- 停滞の原因を仕様変更だけに帰すのは言い過ぎ（実装リソースの事情もある）— この言い方が正確
- 関連: TC39 の **Stage 2.7 新設（2023-11）** の動機説明で Temporal が名指しされた（https://github.com/tc39/notes/blob/main/meetings/2023-11/november-30.md 、Michael Ficarra 発言）
