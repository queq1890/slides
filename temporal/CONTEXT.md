...
- サポート状況セクションは**フル版**（2026-08-20 再構成）: マトリクス → Baseline の定義 → 9 年タイムライン → なぜ①仕様が巨大 → なぜ② Stage 3 でも仕様変更が続いた（Stage 2.7 新設の話込み）→ Safari はなぜ最後になったのか → V8 の Rust 実装 → いつ使えるか。タイムラインを「なぜ」より先に置くのは、経過年数を先に見せないと「なぜ時間がかかったのか」が唐突になるため（2026-08-20 フィードバック）
- npm パッケージセクションは**実践ガイド寄り**: polyfill 2 種比較が主役。既存ライブラリのスタンス表は 2026-08-20 のフィードバックで削除
- デモは 2 本（タイムゾーン変換・型選択）。デモ③（ネイティブ実演）と「学ぶなら今がちょうどいい」系の締め文言は 2026-08-20 のフィードバックで削除
